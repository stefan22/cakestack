...
import { HeroStagger } from '@/components/web/hero-stagger';
...
import PanelAnimation from '@/components/web/panel-animation';
import type * as React from 'react';

export default async function Home() {
  return (
    <>
      {/* Decorative full-screen overlay. Kept a sibling of the content, not a
          parent: a clip-path or transform on an ancestor would become the
          containing block for its position:fixed panels. */}
      <PanelAnimation />
      ...
      <div className="flex mb-12 flex-col">
        ...
        <section className="relative mx-auto overflow-hidden max-w-7xl px-0 pt-16 text-center sm:pt-24">
          <Particles
            ...
            size={0.5}
          />
          {/* Held until the overlay is most of the way open, but only when
              the overlay is running — see HeroStagger. */}
          <HeroStagger className="relative flex flex-col items-center gap-5">
            <StaggerItem>
            ...
            <StaggerItem yOffset={16}>
              <p className="max-w-4xl text-lg text-muted-foreground">
                Essays and field notes from product craft, and the small
                decisions that make software feel considered.
              </p>
            </StaggerItem>
        ...
        <BlurFade
          inView
          className="mx-auto w-full mb-10 max-w-7xl px-0 sm:px-0">
          <CtaSection />
        </BlurFade>
